...
class WorkspaceManager:
    ...
    async def next_invocation_dir(self, session_slug: str | None = None) -> Path:
        ...
        If *session_slug* is provided the directory is scoped per session:
        ``work/<session_slug>/inv_NNNN/``.  This prevents path collisions when
        two concurrent OS processes run against the same workspace with different
        session slugs.  Without a slug the legacy path ``work/inv_NNNN/`` is
